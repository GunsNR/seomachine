...
interface EngineResult {
  ...
  /** null when this engine produced no observation. */
  score: number | null;
...
interface Provenance {
  mode: string; total: number; live: number; observed: number; coverage: number;
}
    ...
    <div>
      <form onSubmit={onSubmit} className="card p-6 sm:p-8">
        ...
        <p className="mt-3 text-[0.8rem] text-body">
          Runs 4 buyer questions across every connected answer engine. No account needed.
        </p>
      </form>
...
function Results({ result }: { result: CheckResponse }) {
  return (
    <div className="mt-10 space-y-8" aria-live="polite">
      {result.provenance.coverage < 1 && (
        <p className="rounded-xl bg-warn/10 p-4 text-[0.85rem] text-ink ring-1 ring-warn/25">
          <strong>Partial coverage.</strong> {result.provenance.observed} of{' '}
          {result.provenance.total} checks returned an answer. The rates below are calculated over
          what was observed — an engine that could not be reached is left out rather than counted
          as an answer that did not name you.
        </p>
      )}
          ...
          {result.byEngine.map((e) => (
            <li key={e.engine} className="card p-4">
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-surface-alt">
                <div className="h-full rounded-full" style={{ width: `${e.score ?? 0}%`, backgroundColor: e.color }} />
              </div>
              <p className="mt-2 text-[0.78rem] text-body">
                {e.observed
                  ? `Named in ${Math.round((e.mentionRate ?? 0) * 100)}% of answers · cited in ${Math.round((e.citationRate ?? 0) * 100)}%`
                  : `No observation — ${e.reason || e.status}`}
              </p>
            </li>
          ))}
        </ul>
